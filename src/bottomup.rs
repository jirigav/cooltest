...
impl Histogram {
    pub(crate) fn get_hist(bits: &Vec<usize>, data: &[Vec<u8>]) -> Histogram {
        ...
        Histogram { bits: bits.iter().copied().collect(), _bins: hist, sorted_indices: indices, best_division: best_i, z_score: max_z }
    }
}
...
fn phase_one(data: &[Vec<u8>], block_size: usize, deg: usize, k: usize) -> Vec<Histogram>{
    ...
    for bits in (0..block_size).into_iter().combinations(deg) {
        ...
        top_k.sort_unstable_by(|a, b| f64::abs(a.z_score).partial_cmp(&f64::abs(b.z_score)).unwrap());
    }
    top_k
}

fn phase_two(data: &[Vec<u8>], block_size: usize, mut top_k: Vec<Histogram>) -> Vec<Histogram> {
    let mut final_bins: Vec<Histogram> = Vec::new();
    let mut length = top_k[0].bits.len();
    while !top_k.is_empty() && length < 10 {
        length += 1;
        let mut new_top: Vec<Histogram> = Vec::new();

        for (i, hist) in top_k.par_iter().map(|hist| {
            ...
            for bit in 0..block_size{
                ...
                new_bits.push(bit);
                let new_hist = Histogram::get_hist(&new_bits.iter().copied().collect(), data);
                if new_hist.z_score < hist.z_score{
                    continue;
                }

                if best_imp.is_none() || f64::abs(best_imp.as_ref().unwrap().z_score) <  f64::abs(new_hist.z_score){
    ...
    final_bins.extend(top_k.into_iter());
    final_bins
} 
...
pub(crate) fn bottomup(
    ...
    start = Instant::now();
    let mut r = phase_two(data, block_size, top_k);
    println!("Phase two in {:?}", start.elapsed());
    r.sort_unstable_by(|a, b| f64::abs(b.z_score).partial_cmp(&f64::abs(a.z_score)).unwrap());
    println!("{:?}", r[0]);
    r[0].clone()
}
